...
import { DonationDetails, DonationPayload, getDonation, updateDonation } from "@/lib/api/donations";
import { DonationForm } from "../../donation-form";

export function EditDonationClient() {
  ...
  async function handleSubmit(payload: DonationPayload) {
    const updated = await updateDonation(params.id, payload);
    router.push(`/donations/${updated.id}?updated=1`);
  }
      ...
      <div className="flex min-h-screen">
        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <header className="border-b border-stone/80 bg-paper/90 px-4 py-5 md:px-8">
            ...
            <h1 className="mt-1 text-2xl font-bold text-ink md:text-3xl">עריכת תרומה</h1>
          </header>
          <div className="space-y-4 px-4 py-6 md:px-8">
            <Link className="text-sm font-bold text-moss hover:text-ink" href={`/donations/${params.id}`}>
              חזרה לכרטיס
            </Link>
            ...
            {donation ? <DonationForm initialDonation={donation} submitLabel="שמירת שינויים" onSubmit={handleSubmit} /> : null}
          </div>
        </section>
      </div>
    </main>
